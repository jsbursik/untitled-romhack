...
// Solenne Regional Dex order: maps Solenne dex position to national dex number.
// Defined in src/data/solenne_dex_order.c
extern const enum NationalDexOrder sSolenneToNationalOrder[];
#define sHoennToNationalOrder sSolenneToNationalOrder
...
enum HoennDexOrder NationalToHoennOrder(enum NationalDexOrder nationalNum)
{
    ...
    while (hoennNum < (SOLENNE_DEX_COUNT - 1) && sHoennToNationalOrder[hoennNum] != nationalNum)
        hoennNum++;

    if (hoennNum >= SOLENNE_DEX_COUNT - 1)
        return 0;
...
{
    if (!hoennNum || hoennNum >= SOLENNE_DEX_COUNT)
        return 0;
...
{
    if (SpeciesToHoennPokedexNum(species) > SOLENNE_DEX_COUNT)
        return FALSE;
    else
        return TRUE;
}
